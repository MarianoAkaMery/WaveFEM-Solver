              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  //Epsilon term
                  cell_stiffness_matrix(i, j) += 1.0*scalar_product(fe_values.shape_grad(i,q),fe_values.shape_grad(j,q))*fe_values.JxW(q);
                  
                   //b term
                   cell_stiffness_matrix(i, j) += 1.0*fe_values.shape_grad(j,q)[0]*fe_values.shape_value(i,q)*fe_values.JxW(q);

                   //Sigma term
                   cell_stiffness_matrix(i, j) += -1.0*fe_values.shape_value(i,q)*fe_values.shape_value(j,q)*fe_values.JxW(q);
                }
            }
        }
...
void
Parab::assemble_rhs(const double &time)
{
  const unsigned int dofs_per_cell = fe->dofs_per_cell;
  const unsigned int n_q           = quadrature->size();
...
void
Parab::output(const unsigned int &time_step) const
{
...
double
Parab::compute_error(const VectorTools::NormType &norm_type)
{
