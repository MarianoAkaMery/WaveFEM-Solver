...
public:
    ...
    public:
        ...
        value(const Point<dim> &p,
              const unsigned int /*component*/ = 0) const override
        {
            return (1 + 0.25 * M_PI * M_PI) * std::sin(0.5 * M_PI * p[0]) * p[1];
        }
    };
    ...
    public:
        ...
        value(const Point<dim> &p,
              const unsigned int /*component*/ = 0) const override
        {
            return std::sin(0.5*M_PI * p[0]) * p[1];
        }
                 ...
                 const unsigned int /*component*/ = 0) const override
        {
            ...
            result[0] = 0.5* M_PI * std::cos(0.5 * M_PI * p[0]) * p[1];
            result[1] = std::sin(0.5 * M_PI * p[0]);
    ...
    // Constructor.
    Ell(const std::string &mesh_file_name_, const unsigned int &r_)
