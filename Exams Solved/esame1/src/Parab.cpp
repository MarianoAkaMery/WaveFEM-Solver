          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  cell_mass_matrix(i, j) += (fe_values.shape_value(i,q)+SUPGtau*k*fe_values.shape_grad(i,q)[0])*
                                            fe_values.shape_value(j, q) /
                                            deltat * fe_values.JxW(q);

                  //First term
                  cell_stiffness_matrix(i, j) += (1.0+SUPGtau*k*k)*scalar_product(fe_values.shape_grad(i,q),fe_values.shape_grad(j,q))*fe_values.JxW(q);
                  
                   //k term
                   cell_stiffness_matrix(i, j) += k*fe_values.shape_grad(j,q)[0]*fe_values.shape_value(i,q)*fe_values.JxW(q);
                }
            }
        }
