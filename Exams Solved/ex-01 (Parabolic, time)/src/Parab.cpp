        ...
        for (unsigned int q = 0; q < n_q; ++q)
        {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                    ...
                    cell_stiffness_matrix(i, j) += (mu_loc * scalar_product(
                                                                 fe_values.shape_grad(i, q),
                                                                 fe_values.shape_grad(j, q)) +
                                                    (0.1 * fe_values.shape_grad_component(i, q, 0)[0] +
                                                     0.2 * fe_values.shape_grad_component(i, q, 1)[0]) *
                                                        fe_values.shape_value(j, q)) *
                                                   fe_values.JxW(q);
                }
            }
...
void Parab::assemble_rhs(const double &time)
{
    const unsigned int dofs_per_cell = fe->dofs_per_cell;
    const unsigned int n_q = quadrature->size();
...
void Parab::output(const unsigned int &time_step) const
{
